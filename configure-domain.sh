...
sudo cp conf/serverdomain.conf /etc/nginx/sites-available/
sudo sed -i "s#/var/www#${HOME}#g" /etc/nginx/sites-available/serverdomain.conf
sudo sed -i "s#varclient#${varclient}#g" /etc/nginx/sites-available/serverdomain.conf
sudo sed -i "s#varap#${varap}#g" /etc/nginx/sites-available/serverdomain.conf
sudo sed -i "s#varsock#${varsock}#g" /etc/nginx/sites-available/serverdomain.conf
sudo ln -s /etc/nginx/sites-available/serverdomain.conf /etc/nginx/sites-enabled/serverdomain.conf
...
cp .env.example .env
sed -i "s#APP_URL=http://localhost/#APP_URL=http://${varclient}/#g" .env
...
cd ../exo-cbt-client-1/
cp .env-example .env
sed -i "s#VUE_APP_URL=#VUE_APP_URL=http://${varclient}#g" .env
sed -i "s#VUE_APP_SOCKET=#VUE_APP_SOCKET=http://${varsock}#g" .env
